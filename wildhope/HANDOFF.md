...
- **Current versions:** app shell `APP_V v24` / SW cache `wildhope-v24`; content.json `version 5` (39 actions, 7 courses, 17 facts, 22 categories, hand-curated `news[]`).
...
**Also done (2026-07-03, same session):** GoatCounter analytics (script tag in `WildHope.html`, per-tab events; site code `wildhope` — owner must register it at goatcounter.com; dashboard at `wildhope.goatcounter.com