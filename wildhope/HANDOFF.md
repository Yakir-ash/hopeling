...
- **Current versions:** app shell `APP_V v35` / SW cache `wildhope-v35`; content.json `version 10` (39 actions, 7 courses, 21 lessons, 17 facts, 26 categories, hand-curated `news[]`).
...
**FEATURE FREEZE + production polish v35 (2026-07-04):** owner accepted PM recommendation: freeze features, focus on distribution readiness. Done this pass: OG/Twitter meta on app + landing with generated `wildhope-web/og-image.png` (PIL, 1200x630); root `index.html` is now a real landing page (was a redirect - bookmark users now land there, one extra tap); `privacy.html` (on-device data, GoatCounter aggregate, Wikipedia/GBIF disclosure) linked from landing footer + Me footer; WHATS-NEW.md changelog in repo. STILL OPEN for production: onboarding audit on real phone (teasers/banners stack for new users), iOS/Safari pass, GoatCounter registration check, Lighthouse audit, real-device QA. Parked: Hope Ledger (waiting for news pipeline to prove itself; owner said not now), all new features. Sandbox gotcha added to the pile: python unicode_escape on strings with emoji surrogate pairs (\uD83C...) produces lone surrogates that never match files - use literal emoji in patch patterns.

**Spirit species quiz v34 (2026-07-04):** growth feature (owner wanted crowd-attractors). `SPIRITS` (12) + `QUIZ` (7 questions, weighted options) in the shell; `spiritScore()` deterministic (first-max in key order); result persisted `state.spirit={id,date}`; result sheet with conservation hook + category link + `shareSpirit()` canvas card; Home teaser until taken/dismissed (`spiritDismissed`), Me row always. Sim brute-forces all 4^7 combos - keep 'all 12 reachable' check green when tuning weights. Future: standalone quiz page on the landing site reusing SPIRITS/QUIZ.

**Home declutter v33 (2026-07-04):** owner flagged crowding. Removed 'Animals needing attention' from Home; daily challenge is now the first row of the (renamed) 'Missions' card via missionsHtml(); eventHtml() returns a one-line banner once the event badge is earned. Home ~10 sections -> ~7. Sim 88 checks.
