...
- **Current versions:** app shell `APP_V v26` / SW cache `wildhope-v26`; content.json `version 5` (39 actions, 7 courses, 17 facts, 22 categories, hand-curated `news[]`).
...
- **Deferred deliberately:** content.json auto-generator (owner shelved it; hand-curated for now); integrity tweaks (day-based badges, diminishing daily XP) — owner explicitly passed (single-player, cheating only hurts yourself). Recommended actual next step: **a week of real daily use + watch GoatCounter**, then let data drive the roadmap. Other parked items: iOS/Safari pass, local content-refresh script.
...
**SW update fix (2026-07-03):** service worker install now fetches the shell with `cache:'reload'` — without it, a new SW could populate its cache from the browser HTTP cache (GitHub Pages `max-age=600`) and permanently serve a stale shell (this bit us: phones stuck on an old version). Cache `wildhope-v14`. Keep `APP_V` (Me footer) in sync with the SW cache 