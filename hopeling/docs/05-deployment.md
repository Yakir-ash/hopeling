...
2. Build & push: `gcloud builds submit --tag gcr.io/PROJECT/hopeling-api`.
3. Deploy:
   ```bash
   gcloud run deploy hopeling-api \
     --image gcr.io/PROJECT/hopeling-api \
     --set-env-vars DATABASE_URL=postgresql+psycopg2://USER:PASS@/hopeling?host=/cloudsql/INSTANCE,AUTH_MODE=firebase,SEED_ON_START=false \
...
6. Lock CORS: `CORS_ORIGINS=["https://admin.hopeling.app"]`.
...
## 3. Android app
1. **Firebase:** create a project → add an Android app (`org.hopeling.app`) → download `google-services.json` into `app/`, and enable the `com.google.gms.google-services` plugin lines in the two Gradle files.
...
## Rollback
Cloud Run keeps revisions - `gcloud run services update-traffic hopeling-api --to-revisions PREV=100`. Android: staged rollout percentages in Play Console; halt if crash-free rate drops.
